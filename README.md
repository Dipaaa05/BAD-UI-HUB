# Bad UI Hub

Have you ever thought that changing your volume was too simple, boring, and lacking in thrills? 
**We fixed that for you.**

This project is a growing collection of mini-games (or rather, psychological tortures) designed with a single goal in mind: to make trivial actions — like turning up the volume — as frustrating, difficult, and irritating as possible.

Welcome to the ultimate hell of User Experience.

## The Collection: Volume Control Edition

Here is our current roster of digital suffering categories. Try to get your volume to 100% without losing your mind:

### Categories:

1. **Casinò**: Luck won't save you here.
2. **Daily Life**: Select your everyday nightmare.
3. **Phisycs**: Gravity and friction are your enemies.
4. **Sport**: Sweat and tears for a decent volume.
5. **Arcade**: Classic fun with terrible UI.
6. **Misc**: Fast paced UI disasters.

## How to Play (or how to drive your friends crazy)

**[Click here to access our site now!](https://baduihub.com)**

## Disclaimer

The creators of this project take **no responsibility** for:
- Mice thrown against the wall.
- Monitors destroyed by punches.
- Nervous breakdowns.
- Swearing and ruined friendships.

*Play at your own risk.*

## License: Look, but don't touch (the money)

The code is released under the **PolyForm Noncommercial License**. 
What does this mean in human terms? You are highly encouraged to read the code, laugh at it, and even contribute to make it worse. But you **cannot** use it for commercial purposes, clone the hub to run your own ads, or monetize our misery in any way. 

We are the only ones allowed to profit from your frustration. See the `LICENSE` file before our lawyers get angry.